...
describe('pin-inventory.yml — fleet observability that cannot lie (EHAC-2845)', () => {
  const INVENTORY = join(import.meta.dirname, '..', '..', '.github', 'workflows', 'pin-inventory.yml');
  const inventory = () => readFileSync(INVENTORY, 'utf8');

  it('exists and runs on schedule + dispatch', () => {
    expect(inventory()).toMatch(/^ {2}workflow_dispatch:/m);
    expect(inventory()).toMatch(/^ {2}schedule:/m);
    expect(inventory()).toMatch(/cron: '[0-9]+ [0-9]+ \* \* [0-9]'/);
  });

  it('runs read-only', () => {
    const perms = inventory().match(/permissions:([\s\S]*?)(\n[a-z]+:|\njobs:)/);
    expect(perms).not.toBeNull();
    expect(perms[1]).toMatch(/contents: read/);
    expect(perms[1]).not.toMatch(/write/);
  });

  it('declares the seven-consumer matrix from EHAC-2845', () => {
    const declared = inventory().match(/^  CONSUMERS: '(.*)'$/m);
    expect(declared).not.toBeNull();
    const repos = declared[1].split(',').map((s) => s.trim()).filter(Boolean);
  ...
  it('reports UNVERIFIED rather than silently skipping repos it cannot read', () => {
    // The vacuous-pass shape this whole gate family prevents: an inventory that said
    // "verified" about unreadable repos would claim fleet completeness it did not measure.
    const src = inventory();
    expect(src).toMatch(/UNVERIFIED \(unreadable\)/);
